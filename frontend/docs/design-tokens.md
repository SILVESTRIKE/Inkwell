...
Font loading: pull Fraunces, Source Serif 4, and Inter as variable fonts via `next/font/google`
in `layout.tsx` rather than a CDN `<link>` — avoids a render-blocking request and gets automatic
...
| Component | Treatment |
|---|---|
| Card (project/character/chapter) | `bg-bg-raised`, `border border-border`, `rounded-md`, `shadow-card`, hover → `shadow-card-hover` + `translate-y-[-1px]`, `duration-base` |
| Primary button (run step) | Solid `bg-accent text-bg`, `rounded-sm`, no pill shape, hover → `bg-accent-hover` |
| Secondary button | Transparent, `border border-border-strong`, text `text-ink`, hover → `bg-accent-soft` |
| Stepper | Numbered serif markers (i, ii, iii…) or plain numerals in `font-serif`, connecting hairline `border-border`, current step in `accent`, done in `success` |
| Error banner | `bg-error-bg`, text `error`, left border-accent 2px `error`, icon: thin outline only |
| Input | `bg-bg`, `border border-border-strong`, `rounded-sm`, focus ring `accent` at 2px, no inset shadow |
